...
public class GetTaxDeductionsQueryValidator : AbstractValidator<GetFederalTaxWithheldQuery>
{
    public GetTaxDeductionsQueryValidator(IConfiguration configuration)
    {
        ...
        RuleFor(p => p.RequestModel.TaxableWageInformation.StateAndFederalTaxableWages)
            .NotEmpty().WithMessage("Federal TaxableWage should not be empty")
            .NotNull().WithMessage("Federal TaxableWage should not be null");
